...
interface SVGGraphVisualizationProps {
  ...
  setContextMenu: React.Dispatch<React.SetStateAction<{ node: GraphNode; x: number; y: number; } | null>>;
}

export default function SVGGraphVisualization({
  supabase,
  graphData,
  setGraphData,
  activeSessionId,
  ...
  }, [graphData, layout, isAggregated, setContextMenu, setSelectedNode]);
  ...
  const handleTrace = async (direction: 'UPSTREAM' | 'DOWNSTREAM') => {
    // This logic should be handled by the parent component, but we'll keep it here for now
    // and assume the parent passes down a function to call
    if (!setContextMenu) return; // a bit of a hack
    setIsLoading(true);
    setError(null);
    // This function needs access to contextMenu, which is not available in this component's state anymore
    // This indicates that handleTrace logic should be lifted up to the parent.
    // For now, we'll comment out the invocation and log a message.
    console.log("Tracing is now handled by the parent component.");
    // The actual trace call is in page.tsx now.
    setIsLoading(false);
  };
  ...
  }, [fetchInitialData, handleExecuteQuery]); // Re-run if fetchInitialData changes

  return (
    <div className="graph-container" ref={containerRef} onClick={() => setContextMenu(null)} style={{ width: '100%', height: '100%' }}>
      <svg ref={svgRef}></svg>
    </div>
  );
}
